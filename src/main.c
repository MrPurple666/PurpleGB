...
typedef struct {
    ...
    int menu_sel, boot_frames; u32 fc, fps_ts; s16 abuf[2048];
} gb_t;

static void re(gb_t*g){mem_init(&g->mem);cpu_init(&g->cpu);timer_init(&g->timer);joypad_init(&g->joypad);ppu_init(&g->ppu);apu_init(&g->apu);g->paused=0;g->boot_frames=0;}
...
int main(int argc,char**argv){
    ...
    while(!gb.quit){
        ...
        if(!gb.paused&&gb.rom_loaded){
            if(gb.boot_frames>0){
                ppu_render_frame(&gb.ppu,&gb.mem);
                gb.boot_frames--;
            }else{
                int tc=0;
                while(tc<70224){
