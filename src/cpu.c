...
int cpu_service_interrupt(cpu_t*c,mem_t*m){u8 p=interrupt_get_pending(m);if(!p)return 0;int b=0;while(!(p&1)){b++;p>>=1;}c->ime=0;c->halted=0;c->sp-=2;mem_write16(m,c->sp,c->pc);c->pc=0x40+(b<<3);m->io[0x0F]&=~(1<<b);return 80;}
